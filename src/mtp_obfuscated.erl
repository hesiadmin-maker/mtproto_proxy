...
client_create(Seed, Secret, Protocol, DcId) when byte_size(Seed) == 58,
                                          ...
                                          DcId =< 32767,
                                          is_atom(Protocol) ->
    %% Base secret mode - use only 16 bytes
    client_create_base(Seed, Secret, Protocol, DcId);
client_create(Seed, FullSecret, Protocol, DcId) when byte_size(Seed) == 58,
                                              byte_size(FullSecret) > 16,
                                              DcId >= -32768,
                                              DcId =< 32767,
                                              is_atom(Protocol) ->
    %% Full secret mode - use entire secret with payload
    client_create_full(Seed, FullSecret, Protocol, DcId).

%% Original client_create (base secret - 16 bytes)
client_create_base(Seed, Secret, Protocol, DcId) ->
...
%% Full secret client_create (with payload)
client_create_full(Seed, FullSecret, Protocol, DcId) ->
    <<L:56/binary, R:2/binary>> = Seed,
    ProtocolBin = encode_protocol(Protocol),
    DcIdBin = encode_dc_id(DcId),
    ...
    %% init_up_encrypt with full secret
    <<_:8/binary, ToRev:(?KEY_LEN + ?IV_LEN)/binary, _/binary>> = Raw,
    <<DecKeySeed:?KEY_LEN/binary, DecIv:?IV_LEN/binary>> = bin_rev(ToRev),
    DecKey = crypto:hash('sha256', <<DecKeySeed:?KEY_LEN/binary, FullSecret/binary>>),

    %% init_up_decrypt with full secret
    <<_:8/binary, EncKeySeed:?KEY_LEN/binary, EncIv:?IV_LEN/binary, _/binary>> = Raw,
    EncKey = crypto:hash('sha256', <<EncKeySeed:?KEY_LEN/binary, FullSecret/binary>>),
...
%% MODIFIED: Checks require_full_secret config
-spec from_header(binary(), binary()) -> {ok, integer(), mtp_codec:packet_codec(), codec()}
                                             | {error, unknown_protocol | full_secret_required}.
from_header(Header, Secret) when byte_size(Header) == 64 ->
    case application:get_env(?APP, require_full_secret, off) of
        on ->
            %% Require full secret mode
            from_header_require_full(Header, Secret);
        off ->
            %% Original mode - base secret only (backward compatible)
            from_header_original(Header, Secret)
    end.

%% Original from_header (base secret - first 16 bytes)
from_header_original(Header, Secret) when byte_size(Header) == 64 ->
...
%% MODIFIED: Requires full secret with payload
from_header_require_full(Header, FullSecret) when byte_size(Header) == 64 ->
    case byte_size(FullSecret) > 16 of
        true ->
            %% Try to decrypt with full secret first
            case try_decrypt_header(Header, FullSecret) of
                {ok, DcId, Protocol, Codec} ->
                    %% Success - client used full secret
                    {ok, DcId, Protocol, Codec};
                {error, _} ->
                    %% Full secret decryption failed
                    %% Check if base secret works (client used short secret)
                    <<BaseSecret:16/binary, _/binary>> = FullSecret,
                    case try_decrypt_header(Header, BaseSecret) of
                        {ok, _, _, _} ->
                            %% Client used only base secret - REJECT!
                            {error, full_secret_required};
                        {error, _} ->
                            %% Neither works - invalid secret
                            {error, unknown_protocol}
                    end
            end;
        false ->
            %% Server secret has no payload - use as is (backward compatible)
            from_header_original(Header, FullSecret)
    end.

%% Helper: try to decrypt header with given secret
try_decrypt_header(Header, Secret) ->
    try
        {EncKey, EncIV} = init_up_encrypt_with_secret(Header, Secret),
        {DecKey, DecIV} = init_up_decrypt_with_secret(Header, Secret),
        St = new(EncKey, EncIV, DecKey, DecIV),
        {<<_:56/binary, Bin1:6/binary, _:2/binary>>, <<>>, St1} = decrypt(Header, St),
        case get_protocol(Bin1) of
            {error, unknown_protocol} = Err ->
...
%% Original init functions - extract first 16 bytes from secret
init_up_encrypt(Bin, Secret) ->
    <<BaseSecret:16/binary, _/binary>> = case byte_size(Secret) > 16 of
                                            true -> Secret;
                                            false -> <<Secret/binary, 0:128>>
                                        end,
    init_up_encrypt_with_secret(Bin, BaseSecret).
...
init_up_decrypt(Bin, Secret) ->
    ...
    init_up_decrypt_with_secret(Bin, BaseSecret).

%% Generic init functions - use whatever secret length is provided
init_up_encrypt_with_secret(Bin, Secret) ->
    <<_:8/binary, ToRev:(?KEY_LEN + ?IV_LEN)/binary, _/binary>> = Bin,
    Rev = bin_rev(ToRev),
    <<KeySeed:?KEY_LEN/binary, IV:?IV_LEN/binary>> = Rev,
    Key = crypto:hash('sha256', <<KeySeed:?KEY_LEN/binary, Secret/binary>>),
    {Key, IV}.

init_up_decrypt_with_secret(Bin, Secret) ->
...
full_secret_test() ->
    %% Test with full secret (any secret with payload)
    FullSecret = crypto:strong_rand_bytes(32), % 16 bytes base + 16 bytes payload
    DcId = 4,
    Protocol = mtp_secure,
    {Packet, _, _, _CliCodec} = client_create_full(crypto:strong_rand_bytes(58), FullSecret, Protocol, DcId),
    Srv = from_header_require_full(Packet, FullSecret),
    ?assertMatch({ok, DcId, Protocol, _}, Srv).

full_secret_reject_short_test() ->
    %% Test that short secret is rejected when full secret is required
    FullSecret = crypto:strong_rand_bytes(32), % 16 bytes base + 16 bytes payload
    <<BaseSecret:16/binary, _/binary>> = FullSecret,
    DcId = 4,
    Protocol = mtp_secure,
    {Packet, _, _, _} = client_create_base(crypto:strong_rand_bytes(58), BaseSecret, Protocol, DcId),
    Srv = from_header_require_full(Packet, FullSecret),
    ?assertMatch({error, full_secret_required}, Srv).
